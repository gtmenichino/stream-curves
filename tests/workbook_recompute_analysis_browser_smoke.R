...
if (!requireNamespace("shinytest2", quietly = TRUE)) {
  cat("workbook_recompute_analysis_browser_smoke: SKIPPED (shinytest2 not installed)\n")
...
if (!requireNamespace("chromote", quietly = TRUE) ||
  ...
  cat("workbook_recompute_analysis_browser_smoke: SKIPPED (chromote/processx/httpuv not installed)\n")
  quit(save = "no", status = 0L)
}
...
if (is.na(chrome_path) || !nzchar(chrome_path)) {
  cat("workbook_recompute_analysis_browser_smoke: SKIPPED (no Chrome/Edge executable found)\n")
  quit(save = "no", status = 0L)
}
...
wait_until <- function(predicate, timeout_sec = 240, interval_sec = 1, label = "condition") {
  start_time <- Sys.time()
  repeat {
...
profile_dir <- tempfile("workbook-recompute-analysis-smoke-profile-")
...
workbook_path <- require_streamcurves_test_workbook("workbook_recompute_analysis_browser_smoke", project_root = project_root)

app <- shinytest2::AppDriver$new(
  app_dir = app_dir,
  name = "workbook_recompute_analysis_browser_smoke",
  load_timeout = 120000
)
...
wait_until(
  function() "summary-recompute_all" %in% names(current_inputs()),
  timeout_sec = 60,
  label = "Reference Curves recompute controls"
)

app$click("summary-recompute_all", wait_ = FALSE)
app$wait_for_idle(timeout = 900000)

wait_until(
  function() {
    "summary-open_analysis_perRiffle" %in% names(current_inputs()) &&
      "summary-recompute_all" %in% names(current_inputs()) &&
      !dom_has_text("Recomputing reference curves") &&
      !dom_has_text("Refreshing reference curve analysis table.")
  },
  timeout_sec = 120,
  label = "bulk recompute to finish"
)
...
wait_until(
  ...
  timeout_sec = 60,
  label = "Analysis popup tabs after recompute"
)
...
wait_until(
  ...
  timeout_sec = 120,
  label = "Analysis popup phase outputs after recompute"
)

wait_until(
  function() dom_has_text("workspace-phase-body") &&
    dom_has_text("Metric Settings") &&
    dom_has_text("Exploratory Analysis"),
  timeout_sec = 120,
  label = "Analysis popup rendered body content after recompute"
)
...
wait_until(
  ...
  timeout_sec = 120,
  label = "Analysis popup dynamic phase pages to finish recalculating after recompute"
)

cat("workbook_recompute_analysis_browser_smoke: OK\n")
